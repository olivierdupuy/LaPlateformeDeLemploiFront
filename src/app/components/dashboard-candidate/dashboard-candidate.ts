...
export class DashboardCandidate implements OnInit {
  ...
  private buildCharts(a: any) {
    ...
    if (this.statusCanvas && a.statusBreakdown?.length) {
      ...
      // Passer par une variable : dans un `push` direct, le type du
      // tableau sert de contexte et efface le type precis de la
      // configuration — `cutout`, propre a l'anneau, n'existe alors plus.
