...
if ($recu -ne $TemoinAttendu) {
  ...
  Deux causes possibles, et elles se distinguent a l'oeil :
    ...
    · une erreur 404.3 — la correspondance de type pour les fichiers
      sans extension manque au « web.config » du dossier de validation,
      « public/.well-known/acme-challenge/web.config » dans le depot du
      front. C'est le defaut qui empechait toute validation avant aout
      2026.
"@
    exit 1
}
